...
def show():
    ...
    st.markdown(
        "This stage performs data cleaning and filtering. Use the options below to select features and algorithms."
    )
    ...
    if run_button:
        ...
        st.session_state["output"] = run_preprocessing(
            metadata_input, feats=selected_feats, algos=selected_algos
        )
    ...
    if st.session_state.get("ran_preprocessing", False):
        ...
        use_log_scale = st.checkbox(
            "Use logarithmic scale (recommended for large value ranges)", value=False
        )
        ...
        sns.boxplot(data=feature_df, ax=ax, showfliers=show_outliers)
        ...
        st.caption(
            "ℹ️ Use log scale to better visualize features with extreme values. Outliers may distort box shape."
        )
