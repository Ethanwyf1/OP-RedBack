...
def create_prelim_zip(prelim_output, preprocessing_output):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        pd.DataFrame(transform_data).to_csv(os.path.join(tmp_dir, "feature_parameters.csv"), index=False)
        ...
        pd.DataFrame(algo_data).to_csv(os.path.join(tmp_dir, "algorithm_performance.csv"), index=False)
        
        # Save best algorithm counts
        best_algo_counts = np.bincount(prelim_output.p.astype(int), minlength=prelim_output.y.shape[1]+1)[1:]
        best_algo_data = []
        for i, algo in enumerate(algo_labels):
        ...
        pd.DataFrame(best_algo_data).to_csv(os.path.join(tmp_dir, "best_algorithm_counts.csv"), index=False)
        ...
        with open(os.path.join(tmp_dir, "analysis_parameters.txt"), 'w') as f:
            ...
            if hasattr(prelim_output, 'max_perf'):
                f.write(f"Performance Direction: {'Maximize' if prelim_output.max_perf else 'Minimize'}\n")
            if hasattr(prelim_output, 'abs_perf'):
                f.write(f"Threshold Type: {'Absolute' if prelim_output.abs_perf else 'Relative (%)'}\n")
...
def display_algorithm_comparison(prelim_output, preprocessing_output):
    ...
    bars1 = sns.barplot(x='Algorithm', y='Good Performance (%)', data=algo_df, ax=ax1, palette='viridis')
    ...
    st.dataframe(algo_df.style.format({
        'Good Performance (%)': '{:.2f}%', 
        'Best Algorithm (%)': '{:.2f}%'
    }))
...
def display_algorithm_performance_heatmap(prelim_output, preprocessing_output):
    ...
    sns.heatmap(heatmap_df, cmap="YlGnBu", cbar_kws={'label': 'Performance (0=Poor, 1=Good)'}, ax=ax)
...
def display_feature_importance(prelim_output, preprocessing_output):
    ...
    sns.heatmap(filtered_df, cmap="viridis", annot=True, fmt=".2f", 
